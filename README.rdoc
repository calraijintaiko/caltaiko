...
3. Install homebrew (on Mac), with the command
...
6. To keep track of any changes made to the original repository, add remote named upstream

   <tt>git remote add upstream https://github.com/greytoyota/caltaiko.git</tt>
...
6. If on a Mac, run the command <tt>brew install imagemagick</tt> to install ImageMagick.
   ...
   Note: it is possible to download ImageMagick by simply running <tt>sudo apt-get install imagemagick</tt>, but if you do so the binaries will be downloaded to <tt>/usr/bin/</tt>, rather than <tt>/usr/local/bin/</tt>, requiring more configuration later and loss of compability with any future changes.
...
8. On Linux, run the command <tt>sudo apt-get install bundler</tt>
...
To sync any changes I have made with your repository, run the commands

   <tt>git fetch upstream</tt>
   <tt>git merge upstream/master</tt>
